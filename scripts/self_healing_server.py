...
def get_high_usage_processes(cpu_threshold=90, mem_threshold=90):
    ...
    for proc in psutil.process_iter(attrs=['pid', 'name', 'cpu_percent', 'memory_percent']):
        try:
            if proc.info['cpu_percent'] > cpu_threshold or proc.info['memory_percent'] > mem_threshold:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--mem", type=int, default=90, help="Memory usage threshold (%)")
    parser.add_argument("--interval", type=int, default=10, help="Monitoring interval (seconds)")
    parser.add_argument("--dry-run", action="store_true", help="Log actions instead of executing them")
