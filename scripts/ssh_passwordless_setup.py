...
def setup_passwordless_auth(host: str, username: str, password: str, public_key: str, port: int = 22) -> bool:
    """
    ...
    try:
        ...
        try:
            ...
            print(f"  ⚠ Warning: Could not verify passwordless auth for {host}: {str(e)}")
        
        ssh.close()
        return True
        
    except paramiko.AuthenticationException:
        print(f"  ✗ Authentication failed for {host}. Please check username and password.")
        return False
...
def main():
    ...
    for server in servers:
        ...
        if setup_passwordless_auth(server, username, password, public_key_content, port):
